#!/usr/bin/env python3
"""Upload One-DM-ckpt.pt and download SD1.5 VAE into Modal Volume onedm-weights."""
...
CANDIDATES = [
    REPO_ROOT / "model_zoo" / "One-DM-ckpt.pt",
    REPO_ROOT / "model_zoo" / "onedm_gdrive" / "One-DM-ckpt.pt",
    REPO_ROOT / "One-DM-ckpt.pt",
]
...
app = modal.App("onedm-populate-weights", image=populate_image)
...
def main():
    ...
    if ckpt is None:
        raise SystemExit(
            "Missing One-DM-ckpt.pt. Download from the One-DM model zoo into "
            "model_zoo/One-DM-ckpt.pt (see README)."
        )

    print(f"Uploading {ckpt} ({ckpt.stat().st_size / 1e9:.2f} GB) to volume {VOLUME_NAME}…")
    with volume.batch_upload(force=True) as batch:
        batch.put_file(str(ckpt), "One-DM-ckpt.pt")
